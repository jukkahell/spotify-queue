...
import { CurrentTrack, Queue, QueueDao, QueueItem } from "./queue";
...
class QueueService {
    ...
    public getCurrentTrack(passcode: string, user: string,
                           onSuccess: (accessToken: string, currentTrack: CurrentTrack, isPlaying: boolean) => void,
                           onError: () => void) {
        this.getQueue(passcode).then(queueDao => {
            const queue: Queue = queueDao.data;
