...
export class Settings extends React.Component<IShareProps, IShareState> {
  ...
  public constructor(props: IShareProps) {
    ...
    this.togglePerkShop = this.togglePerkShop.bind(this);
  ...
  public togglePerkShop(e: React.MouseEvent<HTMLElement>) {
    e.preventDefault();
    if (!this.props.settings.gamify) {
      return;
    }
    const settings = this.props.settings;
    settings.usePerkShop = !settings.usePerkShop;
    this.props.updateSettings(settings);
  }
  ...
  public renderSettingsOptions() {
    return <>
      ...
      <a className={"dropdown-item settingsMenuItem" + (!this.props.settings.gamify ? " disabled" : "")} key="usePerkShop" href="#" id="usePerkShop" onClick={this.togglePerkShop}>
        <FontAwesomeIcon icon="store" />
        <span className="settingName">Use perk shop</span>
        <FontAwesomeIcon className={"settingOptionCheckmark " + (this.props.settings.usePerkShop ? "active" : "inactive")} icon="check-circle" />
      </a>,
